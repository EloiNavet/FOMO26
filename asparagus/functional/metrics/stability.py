...
def compute_gradient_metrics(model: nn.Module, grad_clip_value: Optional[float] = None) -> Dict[str, float]:
    ...
    for p in model.parameters():
        if p.grad is not None:
            ...
            num_parameter_tensors += 1
            num_scalar_parameters += p.numel()
    ...
    return {
        ...
        # Preserve the historical key; it counts tensors, not scalar parameters.
        "num_params_with_grad": num_parameter_tensors,
        "num_parameter_tensors_with_grad": num_parameter_tensors,
        "num_scalar_parameters_with_grad": num_scalar_parameters,
    }
