...
class LinearProbeModule(BaseModule):
    ...
    def __init__(
        ...
        feature_dim = getattr(self.model, "global_feature_dim", None)
        self.heads = self._make_heads(feature_dim) if feature_dim is not None else None
    ...
    def configure_model(self):
        if self.heads is None:
            ...
            self.heads = self._make_heads(feature_dim)

    def _make_heads(self, feature_dim: int) -> nn.ModuleDict:
        heads = nn.ModuleDict()
        for lr in self.learning_rates:
            head_name = self.lr_to_linear_head_name(lr)
            heads[head_name] = self.make_head(feature_dim, self.num_classes)
        return heads
    ...
    def get_features(self, x: torch.Tensor) -> torch.Tensor:
        with torch.no_grad():
            encode_representations = getattr(self.model, "encode_representations", None)
            if callable(encode_representations):
                return encode_representations(x)["h_global"]

            # Deprecated fallback for legacy backbones that have not adopted encode_representations.
            skips = self.model._encode(x)

        if isinstance(skips, (list, tuple, dict)):
            try:
                return build_h_global(skips)
            except (TypeError, ValueError):
                deepest = skips[-1] if isinstance(skips, (list, tuple)) else next(iter(skips.values()))
        else:
            deepest = skips
