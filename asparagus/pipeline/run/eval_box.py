...
def get_inference_data_for_dir(path, task_type):
    inference_data = {}
    # Sorted so traversal order is reproducible, and a second prediction for the same dataset is
    # an error rather than a silent overwrite: os.listdir returns arbitrary filesystem order, so
    # whichever file came last used to win and the reported checkpoint depended on the machine.
    for file in sorted(os.listdir(path)):
        if not file.endswith(".json"):
            continue
        dataset, split, ckpt = get_prediction_metadata_from_path(file)
        if dataset in inference_data:
            raise ValueError(
                f"ambiguous prediction identity in {path}: dataset {dataset!r} has more than one "
                f"prediction file. Exactly one checkpoint role may be evaluated per run."
            )
